# E1T4 — Admin purchase notifications

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ENSTAR

Send admin notification to TELEGRAM_ADMIN_CHAT_ID with user info and transaction ID after successful purchase. Write dialog spec to tests/specs/E1T4.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E1T4.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
