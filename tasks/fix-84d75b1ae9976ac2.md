# fix-84d75b1ae9976ac2 — E6T1: 'Active users' metric not implemented

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ENSTAR

The E6T1 spec explicitly lists "active users" as a metric for the admin `/stats` command. The implementation only tracks `totalStarts` — a counter of total `/start` invocations — which is not the same as distinct active users. No unique-user tracking exists in the stats data model (`src/storage.ts:81-84`), which only holds `totalStarts` and `totalSales`. The admin stats response labels this as "Total /start invocations" which is honest but does not fulfill the "active users" requirement.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-84d75b1ae9976ac2.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-84d75b1ae9976ac2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
